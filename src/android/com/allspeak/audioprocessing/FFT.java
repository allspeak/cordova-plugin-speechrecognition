...
public final class FFT 
{
    ...
    */
    public double[] calculateFFTMagnitude(float inputData[]) 
    {
        int i,n;
        n=1<<this.logm;
        ...
        mag[0] = (double)x[0]; //DC frequency must be positive always

        if (n==1) {
            return mag;
        }
        mag[n/2] = (double) Math.abs(x[n/2]); //pi (meaning: fs / 2)

        //System.out.println("FFT before magnitude");
        //IO.DisplayVector(x);

        for (i=1;i<n/2;i++) {
            mag[i] = (double) Math.sqrt(x[i]*x[i]+x[n-i]*x[n-i]);
  ...
  */
  ...
  public double[] calculateFFTPower(float inputData[]) {
        int i,n;
        n=1<<this.logm;
