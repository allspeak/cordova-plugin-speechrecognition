// Version 1.0.1
// modified by A. Inuggi
// Added read/writeFrames methods for float input
...
public class FeaturesFile
{
    ...
    private int bytesPerSample;		// Number of bytes required to store a feature  ()
    private long numFrames;		// Number of frames within the data section       (varies)
    private long numFeaturesXFrame;     // Number of features contained within each frame (72)
    private FileOutputStream oStream;	// Output stream used for writting data
    private FileInputStream iStream;	// Input stream used for reading data


    // Features Header
    private int numFeatures;		// 2 bytes unsigned, 0x0001 (1) to 0xFFFF (65,535)
    ...
    public int readFrames(float[] sampleBuffer, int offset, int numFramesToRead) throws IOException, Exception
    {
        ...
        for (int f=0 ; f<numFramesToRead ; f++)
        {
            ...
            for (int c=0 ; c<numChannels ; c++)
            {
                sampleBuffer[offset] = (float)floatOffset + (float) readSample() / (float)floatScale;
                offset ++;
            }
    ...
    public int readFrames(float[][] sampleBuffer, int offset, int numFramesToRead) throws IOException, Exception
    {
        ...
        for (int f=0 ; f<numFramesToRead ; f++)
        {
            ...
            for (int c=0 ; c<numChannels ; c++) sampleBuffer[c][offset] = (float)floatOffset + (float) readSample() / (float)floatScale;
    ...
    public int writeFrames(float[] sampleBuffer, int numFramesToWrite) throws IOException, Exception
    {
        return writeFrames(sampleBuffer, 0, numFramesToWrite);
    }
